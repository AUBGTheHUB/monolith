...
def test_participant_dump_as_json(mock_admin_participant: Participant, mock_normal_team: Team) -> None:
    ...
    assert json_data["team_id"] == str(mock_normal_team.id)
...
def test_participant_dump_as_mongo_db_document(mock_admin_participant: Participant, mock_normal_team: Team) -> None:
    ...
    assert mongo_document["team_id"] == mock_normal_team.id
...
def test_team_dump_as_json(mock_normal_team: Team) -> None:

    json_data = mock_normal_team.dump_as_json()

    assert json_data["id"] == str(mock_normal_team.id)
    assert json_data["name"] == TEST_TEAM_NAME
    assert json_data["is_verified"] is False
    assert json_data["created_at"] == mock_normal_team.created_at.strftime("%Y-%m-%d %H:%M:%S")
    assert json_data["updated_at"] == mock_normal_team.updated_at.strftime("%Y-%m-%d %H:%M:%S")


def test_team_dump_as_mongo_db_document(mock_normal_team: Team) -> None:

    mongo_document = mock_normal_team.dump_as_mongo_db_document()

    assert mongo_document["_id"] == mock_normal_team.id
    assert mongo_document["name"] == TEST_TEAM_NAME
    assert mongo_document["is_verified"] is False
    assert mongo_document["created_at"] == mock_normal_team.created_at
    assert mongo_document["updated_at"] == mock_normal_team.updated_at

...
class HackathonService:
    ...
    async def create_random_participant(self, input_data: ParticipantRequestBody) ->  Result[Tuple[Participant, None], DuplicateEmailError | DuplicateTeamNameError | Exception]:
    ...
    async def check_capacity_register_random_participant_case(self) -> bool:
        """Calculate if there is enough capacity to register a new random participant. Capacity is measured in max number 
        of verified teams in the hackathon. This is the Capacity Check 1 from the Excalidraw 'Adding a participant workflow' """

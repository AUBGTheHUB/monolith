...
def register_mentors_routes(http_handler: MentorsHandlers) -> APIRouter:
    ...
    mentors_router.add_api_route(
        "/{mentor_id}", endpoint=http_handler.update_mentor, methods=["PATCH"], dependencies=[Depends(is_authorized)]
    )
    mentors_router.add_api_route(
        "/{mentor_id}", endpoint=http_handler.delete_mentor, methods=["DELETE"], dependencies=[Depends(is_authorized)]
    )

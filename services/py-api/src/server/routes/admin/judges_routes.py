...
def register_judges_routes(http_handler: JudgesHandlers) -> APIRouter:
    ...
    judges_router.add_api_route(
        "/{judge_id}", endpoint=http_handler.update_judge, methods=["PATCH"], dependencies=[Depends(is_authorized)]
    )
    judges_router.add_api_route(
        "/{judge_id}", endpoint=http_handler.delete_judge, methods=["DELETE"], dependencies=[Depends(is_authorized)]
    )

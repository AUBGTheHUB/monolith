...
def register_hub_members_routes(http_handler: HubMembersHandlers) -> APIRouter:
    ...
    hub_members_router.add_api_route(
        "", endpoint=http_handler.get_all_hub_members, methods=["GET"], dependencies=[Depends(is_authorized)]
    )
    hub_members_router.add_api_route(
        "/{member_id}", endpoint=http_handler.get_hub_member, methods=["GET"], dependencies=[Depends(is_authorized)]
    )
    hub_members_router.add_api_route(
        "", endpoint=http_handler.create_hub_member, methods=["POST"], dependencies=[Depends(is_authorized)]
    )
    hub_members_router.add_api_route(
        "/{member_id}",

...
def register_sponsor_routes(http_handler: SponsorsHandlers) -> APIRouter:
    ...
    sponsors_router.add_api_route(
        "", endpoint=http_handler.get_all_sponsors, methods=["GET"]
    )
    sponsors_router.add_api_route(
        "/{sponsor_id}", endpoint=http_handler.get_sponsor, methods=["GET"]
    )
    sponsors_router.add_api_route(
        "", endpoint=http_handler.create_sponsor, methods=["POST"]
    )
    sponsors_router.add_api_route(
        "/{sponsor_id}", endpoint=http_handler.update_sponsor, methods=["PATCH"]
    )
    sponsors_router.add_api_route(
        "/{sponsor_id}", endpoint=http_handler.delete_sponsor, methods=["DELETE"]
    )

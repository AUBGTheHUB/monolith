...
def register_sponsor_routes(http_handler: SponsorsHandlers) -> APIRouter:
    ...
    sponsors_router.add_api_route(
        "", endpoint=http_handler.create_sponsor, methods=["POST"], dependencies=[Depends(is_auth)]
    )
    sponsors_router.add_api_route(
        "/{sponsor_id}", endpoint=http_handler.update_sponsor, methods=["PATCH"], dependencies=[Depends(is_auth)]
    )
    sponsors_router.add_api_route(
        "/{sponsor_id}", endpoint=http_handler.delete_sponsor, methods=["DELETE"], dependencies=[Depends(is_auth)]
    )

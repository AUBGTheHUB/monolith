...
def _get_ssl_config() -> Tuple[str, str]:
    ...
    # This must match the docker-swarm-stack.yaml volume mount target
    caddy_certs_base_path = "/mnt/caddy_certs"

    # The directory names comes from the ACME Issuer ised (in our case CA)
    # See: https://github.com/caddyserver/caddy/blob/a76d005a94ff8ee19fc17f5409b4089c2bfd1a60/modules/caddytls/acmeissuer.go#L48
    # And the string is formatted here:
    # See: https://github.com/caddyserver/caddy/blob/a76d005a94ff8ee19fc17f5409b4089c2bfd1a60/modules/caddypki/acmeserver/acmeserver.go#L262C1-L263C1
    caddy_specific_cert_path = (
        f"{caddy_certs_base_path}/caddy/certificates/acme-v02.api.letsencrypt.org-directory/{DOMAIN}"
    )

    # The domain !="localhost" is needed as when simulating a DEV/PROD env locally we use (ENV=DEV && DOMAIN=localhost)
    # along with the self-signed certs. For DEV/PROD we use the certs generated by Caddy.

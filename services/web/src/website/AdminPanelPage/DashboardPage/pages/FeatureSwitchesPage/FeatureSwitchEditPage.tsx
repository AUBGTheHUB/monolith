...
export default function FeatureSwitchEditPage() {
    ...
    const item = useMemo(() => (id ? getFeatureSwitchById(id) : undefined), [id]);
    ...
    const fsId: string = id; // <-- force-narrow after the guard

    const [name, setName] = useState(item.name);
    const [currentState, setCurrentState] = useState<boolean>(item.currentState);
    ...
    function onSubmit(e: React.FormEvent) {
        ...
        updateFeatureSwitch(fsId, { name: trimmed, currentState });
        window.location.assign('/dashboard/feature-switches');
    }

    function onDelete() {
        if (confirm('Delete this feature switch?')) {
    ...
    return (
        <div className="p-6 md:p-8 grid place-items-center">
            <div className="w-full max-w-lg rounded-2xl border p-6">
                <h2 className="text-xl font-semibold text-center mb-6">Edit feature switch</h2>

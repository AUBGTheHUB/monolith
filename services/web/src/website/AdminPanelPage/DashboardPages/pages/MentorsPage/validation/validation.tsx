...
// 2. Helper for shared file validation logic (size/type checks)
const avatarFileValidation = z
    .any()
...
// 3. Schema for POST (Creation) - Avatar is REQUIRED
export const createMentorSchema = baseSchema.extend({
    avatar: avatarFileValidation.refine((files) => files?.length > 0, 'Avatar file is required'),
});

// 4. Schema for PATCH (Update) - Avatar is OPTIONAL
export const updateMentorSchema = baseSchema.extend({
    avatar: avatarFileValidation.optional(),
});

// Export a union type for the form components to use
export type MentorFormData = z.infer<typeof createMentorSchema> | z.infer<typeof updateMentorSchema>;
